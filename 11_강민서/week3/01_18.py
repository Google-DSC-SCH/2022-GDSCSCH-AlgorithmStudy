...
def solution(k, score):
    singer=[] #명예의 전당 리스트
    result=[] #최소값 리스트
    for i in score[:k]: #k까지는 명예의 정당에 추가하고 최소값 저장
        singer.append(i)
        result.append(min(singer))
    for i in score[k:]: #k이후부터는 
        print(i,min(singer))
        if i>min(singer):
            singer[singer.index(min(singer))]=i
        result.append(min(singer))
    return result
print(solution(9, [10, 30, 40, 3, 0, 20, 4]))